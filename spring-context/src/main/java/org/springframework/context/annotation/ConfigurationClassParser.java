...
class ConfigurationClassParser {
	...
	/**
	 * configClass、sourceClass本质处理AppConfig类
	 *
	 * @param configClass
	 * @throws IOException
	 */
	protected void processConfigurationClass(ConfigurationClass configClass) throws IOException {
		...
		do {
			// 处理Configuration Class
			sourceClass = doProcessConfigurationClass(configClass, sourceClass);
		}
		while (sourceClass != null);
		// configurationClasses @import 类没有存放在BeanDefinitionMap中；之后处理再放入BeanDefinitionMap
	...
	protected final SourceClass doProcessConfigurationClass(ConfigurationClass configClass, SourceClass sourceClass)
		 ...
		 *  @ImportSelector
		 * 	普通类
		 *  @ImportBeanDefinitionRegistrar  		Mybatis中使用
		 */
				...
				for (SourceClass candidate : importCandidates) {
					...
					if (candidate.isAssignable(ImportSelector.class)) {
						...
						if (this.deferredImportSelectors != null && selector instanceof DeferredImportSelector) {
							...
							// 如果自定义的Imports中存在循环Imports就需要递归调用
